...
class CivicReporter {
  constructor() {
    this.currentUser = "user1"; // Simulated user ID, can be expanded for auth
    this.issues = [];
  ...
  renderUserReports() {
    ...
    let filtered = this.issues.filter((issue) => {
      ...
      const isUserIssue = issue.userId === this.currentUser || !issue.userId; // Adjust if auth added
    ...
    return `
      <div class="issue-card ${isAdmin ? "admin-card" : ""}" data-issue-id="${issue.id}">
        ...
        <div class="issue-card__meta">
          ...
          ${
            isAdmin ? `<div class="issue-card__meta-item">
